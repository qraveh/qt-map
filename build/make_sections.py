...
def sec9(lang):
    ...
    o.append("\n")
    # 9.4 derived clock
    H("### 9.4 " + ("Derived clock per path — max(gate, readout, transport) vs measured cycle" if en else "Выведенный такт по путям — max(гейт, считывание, транспорт) против измеренного цикла"))
    H(("| Path | gate | readout | transport | derived lower bound | limiter | measured cycle |" if en else "| Путь | гейт | считывание | транспорт | выведенная нижняя граница | ограничитель | измеренный цикл |")+"\n|---|---|---|---|---|---|---|")
    for p in G['paths']:
        cp=p['clock_parts']
        o.append(f"| {p[L]} | {logt(cp['gate'])} | {logt(cp['readout'])} | {logt(cp['transport'])} | **{logt(p['clock_derived'])}** | {p['clock_limiter'] or '—'} | {p['cycle']} |\n")
    o.append("\n" + ("The derived value is a lower bound (one operation of each kind); a real QEC round stacks several gate layers plus reset, so measured cycles sit 2–5× above it on the fast platforms and inside the stated range on the slow ones. The limiter column is the point: superconducting and spin paths are readout-limited, atom paths are transport-limited, laser-gate ion paths are transport-limited and electronic-gate ion paths are gate-limited." if en else
