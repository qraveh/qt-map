...
def sec9(lang):
    ...
    H(("### 9.1 Construction rules\n\n"
       ...
       "**Seven attributes per node (design space, stable):** (a) carrier-nature affinity on the natural ↔ fabricated spectrum (photon = natural particle in engineered modes; donor/defect = intermediate; carrier-agnostic = neutral); (b) the characteristic time the node imposes (log-scale) with the entangling flag deterministic / probabilistic-heralded; (c) measurement mechanism with time bound, destructiveness and mid-circuit capability; (d) mobility/connectivity mechanism; (e) control modality and placement (temperature stage: room temperature / 4 K / millikelvin; a node may carry several, the first being primary); (f) dominant error structure *as the code sees it*; (g) manufacturing technology. A attribute is a property that does not move as records improve; anything that moves with records (fidelity, Λ, counts, cycle times, QBI stage, funding) is an **evaluation-space attribute** — dated, sourced, attached to the node, never used for position. Actors and goals are **annotations** on paths.\n\n"
