...
def sec9(lang):
    ...
    H("## 9. " + ("The technology graph — nodes, attributes, edges, and what the graph says on its own" if en else "Граф технологий — узлы, атрибуты, рёбра и что граф говорит сам"))
    H(("### 9.1 Construction rules\n\n"
       ...
       "**Семь атрибутов узла (пространство проектирования, стабильно):**\n\n- (a) сродство носителя на спектре естественный ↔ изготовленный (фотон = естественная частица в изготовленных модах; донор/дефект = промежуточный; независимые от носителя = нейтральные);\n- (b) характерное время, которое узел навязывает (лог-шкала), с признаком перепутывания детерминированное / вероятностное-heralded;\n- (c) механизм измерения с нижней границей времени, деструктивностью и возможностью mid-circuit;\n- (d) механизм подвижности/связности;\n- (e) модальность управления и его размещение (температурная ступень: комнатная / 4 K / милликельвины; узел может нести несколько, первая — основная);\n- (f) структура доминирующей ошибки *как её видит код*;\n- (g) технология производства.\n\nАтрибут — свойство, которое не сдвигается с ростом рекордов; всё, что сдвигается (fidelity, Λ, счёт, такт, стадия QBI, финансирование), — **атрибут пространства оценки**: датированный, с источником, прикреплён к узлу и никогда не используется для позиционирования. Акторы и цели — **аннотации** на путях.\n\n"
    ...
    READ={'cx_nn':("universal backbone (commodity)","универсальный каркас (commodity)"),'dec_mwpm':("universal backbone (commodity)","универсальный каркас (commodity)"),'ct_base':("shared by all solid-state spin-like carriers","общий для всех твердотельных спиновых носителей"),
          ...
          'code_color':("transversal Cliffords + distillation on three families — a direction §5 under-weighted","трансверсальные Клиффорды + дистилляция на трёх семействах — направление, недооценённое в §5"),'code_magic':("non-Clifford resource on three families","не-клиффордов ресурс на трёх семействах"),
    ...
    # 9.9 representation
    H("### 9.9 " + ("How a 7-attribute, 5-edge-type graph is best shown — and why the map is dynamic" if en else "Как лучше показывать граф с 7 атрибутами и 5 типами рёбер — и почему карта динамическая"))
    o.append(("Options weighed: a force-directed layout (rejected — it destroys the layer semantics and makes the stack unreadable); an adjacency matrix (right for the dense *requires* relation but wrong for a narrative reader — kept as a table); a chord diagram (rejected — no layers); 3D (rejected — occlusion). The chosen form is a **layered map in the metro-map idiom**: the ten layers are columns; the vertical position inside a column is the node's carrier-nature affinity, so the natural/fabricated *diagonal* becomes literally visible and off-diagonal nodes sit where a warm path dips into the cool half or vice versa; platform paths are coloured lines through their stations; a station on several lines is an interchange — the *transfers* degree at a glance; empty slots are hollow dashed stations. Attributes cannot all be painted at once (one hue channel is all a reader has), so the map carries a **lens** that recolours every station by one attribute at a time, and a **parallel-coordinates strip** shows the full seven-vector of every node as a polyline; a **node inspector** keeps the three spaces physically apart — design attributes, dated evaluation attributes, actors/goals. Edges of type *requires / alternatives / conflicts* are drawn only on focus (degree-of-interest), because " + str(len(G['nodes'])) + " nodes × " + str(sum(1 for e in G['edges'] if e['type'] in ('requires','replaces','conflicts'))) + " edges of those three types is a hairball; *defines* edges live in the inspector as dated rows, not lines. **Dynamic is necessary, static is mandatory:** the resting frame is a complete readable map (all paths, all stations) that prints and thumbnails; the interaction adds lenses, focus and filters without which five edge types cannot be read. The equivalent static artefacts — the tables of §7.2–7.8 — are the print fallback." if en else
       "Взвешенные варианты: force-directed раскладка (отвергнута — уничтожает семантику слоёв и делает стек нечитаемым); матрица смежности (правильна для плотного отношения *требует*, но неверна для читателя-нарратива — оставлена как таблица); хордовая диаграмма (отвергнута — нет слоёв); 3D (отвергнуто — окклюзия). Выбранная форма — **слоистая карта в идиоме схемы метро**: десять слоёв — колонки; вертикальная позиция внутри колонки — сродство носителя, так что *диагональ* естественный/изготовленный становится буквально видимой, а off-diagonal узлы стоят там, где тёплая линия ныряет в холодную половину или наоборот; пути платформ — цветные линии через свои станции; станция на нескольких линиях — пересадочная — степень «переносится» с одного взгляда; пустые слоты — полые пунктирные станции. Все атрибуты нельзя закрасить одновременно (у читателя один канал оттенка), поэтому у карты есть **линза**, перекрашивающая станции по одному атрибуту за раз, и **лента параллельных координат**, показывающая полный семимерный вектор каждого узла как ломаную; **инспектор узла** физически разводит три пространства — атрибуты проектирования, датированные атрибуты оценки, акторы/цели. Рёбра типов *требует / альтернативы / конфликтует* рисуются только в фокусе (degree-of-interest), потому что " + str(len(G['nodes'])) + " узлов × " + str(sum(1 for e in G['edges'] if e['type'] in ('requires','replaces','conflicts'))) + " рёбер этих трёх типов — это клубок; рёбра *определяет* живут в инспекторе датированными строками, а не линиями. **Динамика необходима, статика обязательна:** кадр покоя — полная читаемая карта (все пути, все станции), пригодная для печати и миниатюры; интерактив добавляет линзы, фокус и фильтры, без которых пять типов рёбер не прочесть. Эквивалентные статические артефакты — таблицы §7.2–7.8 — резерв для печати.")+"\n\n")
    # 9.10 edge list
