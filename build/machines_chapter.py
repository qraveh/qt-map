...
def sec_machines(lang):
    ...
    o.append(("| Path | The bet | Machines | Devices | Largest gate-capable device (physical qubits) | Best 2Q error among devices |" if en else
              "| Путь | Ставка | Машин | Устройств | Крупнейшее устройство с гейтами (физ. кубитов) | Лучшая 2Q-ошибка среди устройств |") + "\n|---|---|---|---|---|---|\n")
    for pid in [p['id'] for p in G['paths']]:
        ...
        o.append(f"| {PATHS[pid][L]} `{pid}` | {mdcell(t(L, BET.get(pid, ('—', '—'))))} | {len(pm)} | {len(pd)} | " + (f"{mdcell(big['name'])} — {fmt_n(big['q'])}" if big else '—') + " | " + (f"{fmt_e(errs[0][0])} ({mdcell(errs[0][1])})" if errs else '—') + " |\n")
    o.append("\n")
    ...
    o.append((f"*Test.* Median physical qubits per family over all devices and over gate-capable devices ({len(gdev)} of {len(dev)}; analog simulators, arrays without an entangling gate and single-qubit testbeds excluded); the largest gate-capable device per family; cohort medians for the two largest families. *Result.* All devices: " + '; '.join(f"{t(L, FAMN[f])} {fmt_n(meda[f])} (n = {len(allq[f])})" for f in ('ATOM', 'SC', 'ION', 'PHOTON', 'SPIN') if allq[f]) + ". Gate-capable: " + '; '.join(f"{t(L, FAMN[f])} {fmt_n(med[f])} (n = {len(devq[f])})" for f in ('ATOM', 'SC', 'ION', 'PHOTON', 'SPIN') if devq[f]) + (f" — the atoms lead by ×{r_all:.1f} on all devices and ×{r_gate:.1f} on gate-capable ones" if r_all and r_gate else "") + ". Largest gate-capable device: " + '; '.join(f"{t(L, FAMN[f])} {mdcell(bigg[f]['name'])} {fmt_n(bigg[f]['q'])}" for f in ('ATOM', 'SC', 'ION') if bigg.get(f)) + "; largest device of any kind: " + '; '.join(f"{t(L, FAMN[f])} {mdcell(biga[f]['name'])} {fmt_n(biga[f]['q'])}" for f in ('ATOM', 'SC') if biga.get(f)) + ". Cohort medians (gate-capable; n in brackets) — " + '; '.join(f"{t(L, FAMN[f])}: " + ', '.join(f"{y} {fmt_n(v[0])} ({v[1]})" for y, v in coh[f].items()) for f in ('SC', 'ATOM')) + f". *Verdict:* {t(L, VER[h1])}. The atoms' lead in count is real among processors and much larger among trap arrays: the thousands-of-qubits machines are arrays that do not yet run an entangling gate, while the largest gate-capable tweezer processor and the largest transmon lattice are within a factor of a few of each other. The cohort medians move with who enters the register in a given year (small first chips from new entrants) more than with the leaders, and are too thin per cohort to carry a trend on their own.\n\n" if en else
              f"*Проверка.* Медиана физических кубитов по семействам для всех устройств и для устройств с гейтами ({len(gdev)} из {len(dev)}; аналоговые симуляторы, массивы без перепутывающего гейта и однокубитные стенды исключены); крупнейшее устройство с гейтами по семействам; медианы по когортам для двух крупнейших семейств. *Результат.* Все устройства: " + '; '.join(f"{t(L, FAMN[f])} {fmt_n(meda[f])} (n = {len(allq[f])})" for f in ('ATOM', 'SC', 'ION', 'PHOTON', 'SPIN') if allq[f]) + ". С гейтами: " + '; '.join(f"{t(L, FAMN[f])} {fmt_n(med[f])} (n = {len(devq[f])})" for f in ('ATOM', 'SC', 'ION', 'PHOTON', 'SPIN') if devq[f]) + (f" — атомы впереди в ×{r_all:.1f} по всем устройствам и в ×{r_gate:.1f} по устройствам с гейтами" if r_all and r_gate else "") + ". Крупнейшее устройство с гейтами: " + '; '.join(f"{t(L, FAMN[f])} {mdcell(bigg[f]['name'])} {fmt_n(bigg[f]['q'])}" for f in ('ATOM', 'SC', 'ION') if bigg.get(f)) + "; крупнейшее устройство любого рода: " + '; '.join(f"{t(L, FAMN[f])} {mdcell(biga[f]['name'])} {fmt_n(biga[f]['q'])}" for f in ('ATOM', 'SC') if biga.get(f)) + ". Медианы по когортам (с гейтами; n в скобках) — " + '; '.join(f"{t(L, FAMN[f])}: " + ', '.join(f"{y} {fmt_n(v[0])} ({v[1]})" for y, v in coh[f].items()) for f in ('SC', 'ATOM')) + f". *Вердикт:* {t(L, VER[h1])}. Лидерство атомов по числу реально среди процессоров и много больше среди массивов ловушек: машины на тысячи кубитов — это массивы, которые пока не выполняют перепутывающий гейт, тогда как крупнейший пинцетный процессор с гейтами и крупнейшая трансмонная решётка отличаются в считанные разы. Медианы когорт движутся вместе с тем, кто входит в реестр в данном году (малые первые чипы новых участников), а не с лидерами, и слишком тонки по когортам, чтобы нести тренд сами по себе.\n\n"))
