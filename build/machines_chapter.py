...
def sec_machines(lang):
    ...
    o.append((f"*Test.* Machines with at least one code in the register's code table, by cohort 2023–2026; machines whose decoding is in the loop. *Result.* {len(codem)} of {N} machines list a code — {len(codem_dev)} of them devices that have run it, the rest announced or planned machines whose roadmap names one (" + ', '.join(f"{t(L, FAMN[f])} {sum(1 for m in codem if m['family']==f)}" for f in FAM_ORDER if any(m['family']==f for m in codem)) + "); share by cohort " + ', '.join(f"{y} {pct(c[0], c[1])} ({c[0]}/{c[1]})" for y, c in sorted(cohc.items())) + f"; decoding in the loop: {len(inloop)} (" + ', '.join(mdcell(m['name']) for m in sorted(inloop, key=lambda m: m['name'])) + f"). *Verdict:* {t(L, VER[h4])}. The share rises but not monotonically, and the loop is closed on a handful of machines: the entry ticket is a code *demonstration*, not yet a decoder in the cycle.\n\n" if en else
    ...
    o.append("\n")
    o.append((f"Limits. The cohort is the year of the register's status date, which mixes first light, general availability and the latest milestone; the classifiers are string rules over free-text fields, printed at the top of the chapter's generator in the map's repository (`build/machines_chapter.py`) so that a reviewer can object to a rule rather than to a number; the population is the register's, which favours machines with an English-language document. {len(pressonly)} machines have no verified cell at all (press pages only) and enter the counts at press level: " + ', '.join(mdcell(x) for x in pressonly) + ". Every hypothesis is re-tested by each build; a verdict that flips between editions is itself a finding and will be reported in the changelog.\n\n" if en else
