...
// hand over to the page, which scrolls until the map has left the window; at its top the same two notches upward hand over
// the other way — the page scrolls up until the map is below the window (or to the top of the page) (24–25 Sep 2026)
(function(){ const wrap=document.getElementById('mapwrap'), body=document.getElementById('mapbody'); if(!wrap||!body)return; let n=0, dir=0, t=null;
  const scrollable=()=>wrap.scrollHeight>wrap.clientHeight+1;
  const atBottom=()=>scrollable()&&wrap.scrollTop+wrap.clientHeight>=wrap.scrollHeight-1;
  const atTop=()=>scrollable()&&wrap.scrollTop<=0;
  function bump(d,ev){ if(d!==dir){ dir=d; n=0; } n++; clearTimeout(t); t=setTimeout(()=>{n=0;},1500); if(n>=2){ n=0; if(ev&&ev.cancelable)ev.preventDefault(); d>0?goDown():goUp(); } }
  function fs(){ return !!(window.__mapFullscreenOn&&window.__mapFullscreenOn()); }
  function goDown(){ const end=document.querySelector('#mapbody .legend')||wrap;
  ...
  function goUp(){ if(fs()){ body.scrollTo({top:0,behavior:'smooth'}); return; }
    const top=body.getBoundingClientRect().top+window.pageYOffset; const y=top-window.innerHeight-2;   // the map's top edge just below the window
    window.scrollTo({top:Math.max(0,y),behavior:'smooth'}); }
  wrap.addEventListener('wheel',ev=>{ if(ev.deltaY>0){ if(atBottom())bump(1,ev); else n=0; } else if(ev.deltaY<0){ if(atTop())bump(-1,ev); else n=0; } },{passive:false});
