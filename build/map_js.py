...
function drawEdges(){ gEdges.selectAll('*').remove(); const f=state.focus, iso=state.isolate;
  // a selection (isolated path, lens filter, or both) shows every relation among the selected stations (dependencies, alternatives,
  // conflicts) without any toggle; a toggle then adds that type's relations that reach outside the selection (drawn quieter);
  // a focused station always shows all of its own
  const mem=f?null:selectedSet();
...
function inspectPath(p){ const L=lang(); insp.hidden=false;
  ...
  <div class="space"><h4>${T('Relations within the path — drawn on the map','Связи внутри пути — показаны на карте')}</h4>${relHTML}<div class="empty" style="margin-top:3px">${T('the edge toggles add each type\'s relations that reach outside the selection','переключатели рёбер добавляют связи этого типа, выходящие за пределы выбора')}</div></div>
