...
function inspectPath(p){ const L=lang(); insp.hidden=false;
  ...
  const parts=p.clock_parts||{}; const lim=p.clock_limiter?T(...({gate:['gate','гейт'],readout:['readout','считывание'],transport:['transport','транспорт']}[p.clock_limiter]||[p.clock_limiter,p.clock_limiter])):'—';
  ...
  <div class="space"><h4>${T('Clock','Такт')}</h4><dl><dt>${T('derived','производный')}</dt><dd><b>${fmtT(p.clock_derived)}</b> · ${T('limiter','ограничитель')}: ${lim}</dd><dt>${T('parts','части')}</dt><dd>${['gate','readout','transport'].map(k=>`${T(...({gate:['gate','гейт'],readout:['readout','считывание'],transport:['transport','транспорт']})[k])} ${fmtT(parts[k])}`).join(' · ')}</dd><dt>${T('measured','измерено')}</dt><dd>${esc(p.cycle||'—')}</dd></dl></div>
