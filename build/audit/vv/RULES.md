...
17. **Focus + isolate on a path not through focus** — until 20 Sep: empty lit set and no lines (no fallback). Since 21 Sep (adjudication 12): the newer selection releases the older one; the oracle's `effective()` applies the release in the driver's gesture order (isolate, machine, focus).
...
11. 20 Sep: a machine chosen from a station card's "Used by" list becomes the selection — the station focus is released, the machine card opens and the lit set is the machine term alone (before: the focus stayed and the lit set was the intersection with that station's paths, which the editor read as "nothing happens" and as a toggle on a double click). The Machine selector's behaviour is unchanged. Sheet mode is now "narrow (≤ 700 px) or touch up to 1024 px"; a mouse laptop at ≤ 1024 px keeps the floating card, so the 1024-px viewport of the runner exercises the card, not the sheet.

12. 21 Sep: **selections that cannot share a line release each other** (PAGE-BUG by the editor's reproduction: "click the trapped-ions line, then the dual-rail station — the superconducting path is missing"; the same state explains the missing path curve of the 20 Sep screenshot). Rule: the isolated path, the focused station and the machine narrow each other only while they can share a line — the station lies on the isolated path (primary or alternate slot), the machine's path is the isolated path, the machine's path passes through the station. A new selection that cannot share a line with an older one releases the older one (its chip is unpressed / the station is deselected / the machine selector is emptied) instead of leaving the empty intersection. The "Used by" click keeps adjudication 11 (it always releases the focus). The lens filter is not a selection and still intersects (a family value hides the lines of other families; the legend shows the pressed value). Changes: map_js.py `select()`, `isolatePath()`, `setMachine()` (helpers `pathsOf`, `releaseIsolate/Focus/Machine`); oracle.py `effective()` applied inside `expected()`; runner.py M2 picks a compatible path for the identity and adds M2b (an incompatible isolate must release — corrected 23 Sep: only the term that cannot share the new line is released, so a focus whose station lies on the path, or a machine whose path it is, is expected to stay); test_oracle.py invariant: with no lens filter every isolate × focus (and machine × isolate/focus) combination lights ≥ 1 line and the focused station; c2_smoke.py `selections()` replays the editor's steps and the mirror cases. Dimmed line hit-areas keep `pointer-events:none`, so the mirror case (focus first, then a line) is reached through the path chips.
