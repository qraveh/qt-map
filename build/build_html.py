...
<div class="pcwrap" id="pcwrap"><h3 class="lang-en">Technologies across the seven coordinates — a parallel-coordinates view</h3><h3 class="lang-ru">Технологии по семи координатам — вид в параллельных координатах</h3>
<p class="lang-en">Each polyline is one technology (a station of the map, a node of the graph — the three words name the same thing in its three homes: the field, the map, the data); the vertical axes are its seven design coordinates (a)–(g) of §7.1. Hover to name a line, click to select it on the map. Bundles reveal the diagonal (natural carriers run through optical control and transport; fabricated ones through microwave/electrical control and static wiring); lines that cross the bundles are the off-diagonal technologies.</p>
<p class="lang-ru">Каждая ломаная — одна технология (станция карты, узел графа — три слова называют одно и то же в трёх его домах: отрасли, карте, данных); вертикальные оси — её семь координат проектирования (a)–(g) из §7.1. Наведите, чтобы назвать линию; кликните, чтобы выбрать её на карте. Пучки показывают диагональ (естественные носители идут через оптическое управление и транспорт; изготовленные — через СВЧ/электрическое управление и статическую разводку); линии, пересекающие пучки, — off-diagonal технологии.</p>
<div id="pc"></div></div>'''
