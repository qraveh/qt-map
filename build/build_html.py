...
<h2><span class="num">{'MAP' if en else 'КАРТА'}</span>{'Technology map — ten layers, seven attributes, fourteen platform paths' if en else 'Карта технологий — десять слоёв, семь атрибутов, четырнадцать путей платформ'}<button type="button" class="chip mapcol" data-mapcollapse="1" aria-expanded="true"><span class="when-open">{'▾ collapse' if en else '▾ свернуть'}</span><span class="when-closed" hidden>{'▸ expand map' if en else '▸ развернуть карту'}</span></button></h2>
