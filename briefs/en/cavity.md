...
## Role in the stack
Requires machined 3D cavities; provides the mode for cat and GKP encodings, dual-rail erasure, ancilla-mediated gates and dispersive readout. Derived clock = max(gate, readout, transport): ≈1.0 µs on the dual-rail cavity path, the 500 ns CZ sitting under ~1 µs ancilla readout [D][3], and 2.8 µs on the cat path [D][5]. Verification: the 25.6/34 ms figures are single-group and unreplicated [D][2]; the dual-rail CZ's Pauli error is post-selected, and its Λ ≈ 27 is simulated, not measured [S][G:QCI-LAMBDA27-2026-08].
