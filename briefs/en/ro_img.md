...
## Role in the stack
Requires an alkali or alkaline-earth atom with a cycling transition; provides the non-destructive mid-circuit measurement real-time decoding needs, and turns atom loss into detectable erasure, not silent Pauli error. Derived clock = max(gate, readout, transport) ≈ 1×10⁻³ s, set by readout not the 270 ns gate — ~10³× a superconducting cycle. Verification: both microsecond results are single-group and single-species; Kyoto's uses spinless ¹⁷⁴Yb, so it establishes occupancy discrimination, not hyperfine state readout, and USTC's 15 µs averages over a 25-site subarray. Neither has run inside a logical memory.
