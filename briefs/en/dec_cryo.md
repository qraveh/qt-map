...
## Role in the stack
It requires millikelvin SFQ or 4 K cryo-CMOS control first, provides for nothing downstream, and blocks no one. It replaces room-temperature FPGA decoding: an FPGA is reprogrammable as codes and noise models change, a cold ASIC is fixed-function, competes with control for cold watts, and is export-controlled from the design file. The argument for an ASIC is supply-side: merchant high-end FPGAs are an AMD/Altera duopoly, one side private-equity-controlled since 2025 [G:ALTERA-SILVERLAKE-2025]. As a pre-decoder it adds ≈0.10 µs per round [S][G:CRYOZIP-2026-06], leaving derived clock = max(gate, readout, transport) at ≈1.1 µs; only the cabling cost changes.
