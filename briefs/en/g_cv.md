...
## Identity & lineage
Gates act on optical quadratures — beam-splitters, phase shifts, squeezers — closed by homodyne feed-forward. Gaussian operations with homodyne detection are classically simulable, so universality needs a non-Gaussian resource: the Gottesman–Kitaev–Preskill grid state, 2001 [S][329]. Time-domain multiplexing predates the companies — the University of Tokyo entangled over 10,000 modes in 2013 [D][615]; Xanadu added the modular chip version [D][149] and the first integrated GKP source [D][150]. Attributes: flying carrier, partly fabricated, ~1 µs deterministic step, Gaussian-noise and loss-dominated error; electro-optic control at room temperature, photonic-IC fabrication.

## Physics & limits
Loss acts as a Gaussian random-displacement channel on the grid, blurring the GKP peaks until modular-quadrature measurement cannot resolve the logical value; the floor is effective squeezing, not gate count. Xanadu's on-chip source reports 0.62 dB [D][150] — grid-noise variance ~0.87 of vacuum against ~0.10 at the ~10 dB assumed, an ~8.7× reduction still to find [S], a different quantity from Xanadu's 24.1× loss factor above threshold [C][154]. Only lower per-element loss, better sources and deeper multiplexing move it. Feed-forward adds a second floor: electronics must act inside the ~1 µs clock, and the published CV loop is 196 ns [P][616].

## Engineering state of the art

| Date | Figure | Who | Tag |
|---|---|---|---|
| 2013-11 | >10,000-mode time-domain CV cluster state | Univ. of Tokyo | [D][615] |
| 2025-01 | Aurora: 35 chips, 12 modes per 1 MHz cycle, 2 h, ~14 dB loss | Xanadu | [D][149] |
| 2025-06-05 | First integrated GKP source, 0.62 dB effective squeezing, SiN | Xanadu | [D][150] |

No GKP logical qubit exists; Aurora ran two hours without error correction and reported no logical fidelity, so it shows stability, not computation [D][149].

## Manufacturing, materials & supply chain
Fabrication is silicon-nitride lithography on 300 mm-class wafers, shared with the discrete-variable sector; the CV-specific additions are squeezers and high-efficiency homodyne receivers rather than switches, and the GKP source runs at room temperature [D][150]. Packaging is Corning fibre arrays and DISCO singulation [C][148].  Control is the heavy part: every mode needs a homodyne receiver, an ADC and a loop closing inside the clock, scaling linearly with no published shortcut. At 10³ modes that is a rack of receivers, at 10⁴ the analogue front end dominates, and nothing addresses 10⁶. ECCN 4A906 applies to the machine [G:BIS-QUANTUM-2024].

## Role in the stack
The continuous-variable path's gate layer, requiring squeezed sources and room-temperature feed-forward. It substitutes for rather than complements discrete-variable fusion: CV measures quadratures where fusion detects single photons, so switching paths replaces sources, detectors and decoders together. Derived clock 1.0 MHz [D][149]. Verification is thin: the 0.62 dB is single-group, unreplicated, and routinely confused with raw quadrature squeezing — the 1.4 dB in periodically poled TFLN [P][275] is a different quantity.
...
|---|---|---|---|---|
| Xanadu | developer | CA | Aurora, integrated GKP source, GKP+qLDPC roadmap | [D][149][D][150] |
| University of Tokyo | research | JP | Origin of large-scale time-domain CV cluster states | [D][615] |
| Corning | supplier | US | Fibre arrays behind Xanadu's coupling figure | [C][148] |
...
2026-08-28 · Xanadu · Toronto factory funding · CAD 195 M · Government of Canada · announced [C][154]
...
[275] X. Shi *et al.*, “Squeezed Light Generation in Periodically Poled Thin-Film Lithium Niobate Waveguides,” [arXiv:2508.08599](https://arxiv.org/abs/2508.08599), Aug. 2025. [P]
...
[615] S. Yokoyama *et al.*, “Ultra-large-scale continuous-variable cluster states multiplexed in the time domain,” *Nat. Photon.*, vol. 7, no. 12, pp. 982–986, Dec. 2013, doi: [10.1038/nphoton.2013.287](https://doi.org/10.1038/nphoton.2013.287). [D]
