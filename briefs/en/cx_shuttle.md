...
Attributes. **a — affinity:** fully fabricated, an electron in a lithographic ²⁸Si/SiGe well. **b — time:** ~2 × 10⁻⁷ s per 10 µm transit [D][1]; entangling not applicable, this is transport. **c — readout:** none of its own, it borrows the register's charge sensor. **d — mobility:** transport, the node's entire function. **e — control:** no modality or placement of its own, one shared radio-frequency drive. **f — error as the code sees it:** coherent, dephasing and position-dependent rotations, no loss channel. **g — manufacturing:** CMOS, the same 300 mm line as the dots.
