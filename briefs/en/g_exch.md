...
## Role in the stack
It entangles two families: silicon and germanium quantum-dot spins (Intel, Diraq, Quantum Motion, HRL under IBM, Quobly, Equal1) and donor spins (SQC). It requires a gate-defined dot or donor plus baseband control, and provides the entangling layer both families build codes on — at HRL a distance-5 repetition code with Λ(5/3) = 4.7 over 200 rounds, which detects bit flips only and is therefore not a below-threshold result for a full code, plus a post-selected [[4,2,2]] at 0.95 logical fidelity [D][2]. It conflicts with crossbar addressing, calibration being per pair. Derived clock ≈ 1–100 µs per layer, readout-dominated: the gate is not the bottleneck. Neighbouring empty slot: an all-pairs, same-chip result past eight qubits.
