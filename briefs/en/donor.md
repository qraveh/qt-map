...
## Role in the stack
Requires STM hydrogen lithography; provides the carrier for exchange links and spin-to-charge readout; substitutes for gate-defined dots at the cost of the whole process flow. Its clock contribution is not distinctive — µs gates and ~10 µs readout match dot spins, so max(gate, readout, transport) is unchanged and the real bottleneck is a manufacturing quantity the clock cannot express. Verification: one group, one device, no replication, three inconsistent fidelity framings [D][2].
