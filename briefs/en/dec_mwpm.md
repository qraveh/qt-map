...
## Role in the stack
Default real-time decoder for surface- and colour-code memories on the superconducting, ion, photonic and spin paths [graph]; it needs the near-planar graph a static nearest-neighbour lattice supplies free. It replaces neural and belief-propagation decoders, which win once checks stop being matchable; switching is asymmetric — leaving costs a hardware implementation and a new verification story, returning costs nothing. Clock contribution ≈1.0 µs per round at d=17 [D][3] against a derived clock of 1.1 µs for the transmon surface-code path (derived clock = max(gate, readout, transport) for the path). Empty slot nearby: no real-time matcher consumes soft readout or leakage flags.
