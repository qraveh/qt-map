...
A mid-circuit erasure check asks "is this qubit still inside its code space?" and returns a one-bit herald with the qubit's location without disturbing the logical state. It needs an encoding whose dominant decay leaves the code space: dual-rail (one excitation in two transmons or two cavities; loss leaves the detectable vacuum) or a metastable atomic qubit (³P₀ in ¹⁷¹Yb, D₅/₂ in ⁴⁰Ca⁺) whose ground-state decay fluoresces under a beam the qubit does not see. Proposals: Princeton/Yale, 2022-01 — 98% of ¹⁷¹Yb errors convertible, surface-code threshold 0.937%→4.15% [S][1]; AWS dual-rail transmons, 2022-08 [S][2]; Yale cavities, 2022-12 [S][3]. Demonstrations: Princeton ¹⁷¹Yb [D][4] and Caltech ⁸⁸Sr [D][5] in 2023-05; AWS transmons [D][6] and Yale cavities [D][7] in 2023-07.
...
- b = n/a.
- c = ancilla erasure check, 10⁻⁶·⁴ s ≈400 ns, non-destructive, mid-circuit — the transmon record [D][8], not the atomic 20 µs [D][4].
- d = none.
...
In a transmon dual-rail pair the logical states are |10⟩ and |01⟩, so a T₁ decay of either transmon lands in |00⟩. The check is a joint dispersive readout tuned so that both logical states pull the shared resonator equally while the vacuum pulls it differently; AWS reaches a residual logical splitting of −0.7(5) kHz against a −4.25 MHz dispersive shift [D][8]. In a cavity pair an ancilla sensitive to the total photon number does the same job in 1.82 µs [D][9]. For atoms a decay from ³P₀ to ¹S₀ is caught by 20 µs of fluorescence on ¹S₀–¹P₁, invisible to the metastable qubit, so survivors pick up less than 10⁻⁵ error per check [D][4].

Each check costs one exposure to T₁: AWS measures 2.54(1)×10⁻² erasure per 384 ns check against a residual undetected error of 6.0(2)×10⁻⁴; their ratio, the erasure bias of 42(1), is what a code cares about [D][8].

Three floors are intrinsic. False negatives re-enter the code as leakage: Yale's 3.7% false-negative rate times its erasure rate gives a missed-erasure fraction of 9.0(5)×10⁻⁴ [D][9]; AWS's are the readout's ≈0.8% separation error [D][8]. Back-action: residual χ mismatch dephases the qubit by 8(3)×10⁻⁵ per check and the readout tone halves the erasure lifetime while on [D][8]. Time: in atoms the 20 µs check stretches to 420 µs when Rydberg population must decay before imaging — hence 56(4)% of single-qubit but only ≈33% of two-qubit errors are converted [D][4]. The code sees a located erasure p_e, a Pauli residual ≈p_e/40 and an unheralded leakage tail ≈FN×p_e; under that channel the surface-code threshold is 4.15% [S][1] and biased-erasure codes reach 8.2–10.3% [S][10]. The floor moves with longer T₁, stronger readout and, for atoms, faster Rydberg-leakage detection.

## Engineering state of the art

Best demonstrated: one dual-rail transmon qubit checked in 384 ns with a 6×10⁻⁴ residual (2026-04) [D][8]; a cavity CZ with 0.5% erasure and 0.03% Pauli error (2026-08) [D][11]. Typical at scale (3 Sep 2026): nothing above ten erasure qubits — SUSTech's four dual-rail transmons at 98.8% logical Bell fidelity (2025-04) [D][12], Quantum Circuits' 8-qubit Seeker (2024-11) [C][13]. On atoms, Princeton's [[4,2,2]] blocks [D][14] and JILA's 20-atom GHZ state [D][15] (2025-06) are the largest erasure-checked circuits; Microsoft/Atom Computing's 24 logical qubits use end-of-round loss imaging, not mid-circuit checks [D][16].

| Year | Figure | Who | Tag | Src |
|---|---|---|---|---|
| 2023-05 | 20 µs check at 0.986 detection fidelity; 56(4)% of 1Q errors converted | Princeton ¹⁷¹Yb | [D] | [4] |
| 2023-07 | Erasure 2.19(2)×10⁻³ per gate, residual ≈40× lower, <0.1% dephasing per check | AWS transmons | [D] | [6] |
| 2024-06 | 1.82 µs mid-circuit check, FP 0.51%, FN 3.7% | Yale cavities | [D] | [9] |
| 2024-11 | ⁴⁰Ca⁺ Bell 98.56→99.14% with erasure excision; 1 ms check | Oregon | [D] | [17] |
| 2025-06 | ¹⁷¹Yb error-detected CZ 99.78(4)%, >90% Rydberg-decay detection | JILA | [D] | [15] |
| 2025-06 | [[4,2,2]] teleportation 0.771→0.802 with erasure-selected ancillas | Princeton | [D] | [14] |
| 2026-04 | 384 ns check, residual 6.0(2)×10⁻⁴, bias 42(1) | AWS | [D] | [8] |
| 2026-08 | Dual-rail CZ 500 ns, erasure ≈0.5% per gate, Pauli 0.029(6)%, bit-flip 2.8(4)×10⁻⁶ | Quantum Circuits | [D] | [11] |
...
**Strategic reading.** If erasure checks scale, the winners own high-T₁ fabrication and the readout chain — D-Wave, AWS and the alkaline-earth camp (Atom Computing, Google); the losers are bare-transmon roadmaps buying threshold with Λ alone. Substitution threats: qutrit erasure qubits [D][22] remove the 2× overhead; fast Yb imaging [D][20] and loss-aware decoding erode the case for metastable encodings. Bargaining power stays with platform vendors, not suppliers.
