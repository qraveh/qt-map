...
Внедиагональное прочтение: изготавливаемый носитель получает дальнюю связность через корпусирование там, где атомы и ионы получают её через движение. Производный такт = max(гейт, считывание, транспорт) для данного пути: его задаёт считывание за ~0.5 µs, а межмодульный SWAP за < 100 ns [D][7] составляет около 20% от этой величины, так что транспорт не ограничивает. Соседние пустые слоты: межмодульная коррекция ошибок и qLDPC-код, проверки которого охватывают несколько модулей, — компиляция для таких машин существует пока только в моделировании [S][14].
