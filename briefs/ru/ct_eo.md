...
## Физика и пределы
Ограничением служит не полоса модулятора. Связывают руки фронт нарастания детектора, дискриминация, принятие решения и установление драйвера, которые обязаны уместиться в ту оптическую задержку, какую вы способны себе позволить. Удержание фотона на 100 нс в нитриде кремния стоит ~15 м волновода; при измеренных PsiQuantum потерях 1.8 ± 0.2 дБ/м в одномодовом SiN это ~27 дБ против бюджета около 0.5 дБ на фотон. Та же задержка в волокне стоит ~4 мдБ плюс два перехода волокно–кристалл по 52 ± 12 мдБ [D][1]. Линия задержки для упреждающего управления обязана жить вне кристалла; цена её потерь — это корпусирование, а не волновод [S]. Выше ~1 ГГц стеной становится мёртвое время SNSPD.

## Инженерное состояние (state of the art)

| Дата | Показатель | Кто | Тег |
|---|---|---|---|
| 2025-01 | Упреждающее управление за один такт 1 МГц, 35 кристаллов | Xanadu | [D][2] |
| 2026-06-02 | ~150 нс от входа детектора до установившегося воздействия | QuiX Quantum | [C][3] |
| 2026-06-02 | Петля в непрерывных переменных 196 нс | Duggan et al. | [P][4] |
| 2026-06-04 | Энергонезависимая матрица на BTO, 80 нс | Lumiphase | [D][5] |

Июнь 2026 года переписал арифметику: 80 нс против такта 1 мкс — это ~12×, а стоечная электроника с 150–196 нс сокращает запас до ~2×. Нулевая статическая мощность этой матрицы обходится в 1.48 дБ на ячейку, то есть примерно в 15× больше, чем 100 мдБ у проходного коммутатора PsiQuantum [D][1], [5], — плоскость маршрутизации, а не плоскость слияния.

## Производство, материалы и цепочка поставок
TFLN поставляют два коммерческих вендора — HyperLight и Lightium [P][6]; серийный BTO — только 300-мм маршрут PsiQuantum на GlobalFoundries [D][1] и пилотная линия Lumiphase [D][5]; в стыковке Xanadu с 0.085 дБ на грань использовались Corning и DISCO [C][7]. Стоимость в пересчёте на канал публично не раскрыта. Нагрузка на ввод-вывод — это и есть данный узел: драйвер и ЦАП на каждый модулятор, коаксиал и дискриминатор на каждый детектор. Блок QuiX имеет размерность 32×32 [C][3], так что 10³ мод — это ~30 стоек; при 10⁴ стеной становится межстоечный перекос тактов, при 10⁶ не масштабируется уже ничего. Машина подпадает под позицию BIS ECCN 4A906 [G:BIS-QUANTUM-2024].

## Роль в стеке
Это хаб, а не выбор пути платформы: он одинаково обслуживает путь на слияниях (PsiQuantum, Quandela, QuiX) и путь непрерывных переменных/GKP (Xanadu) и задаёт производный такт везде, где итеративно повторяются слияния или гомодинные измерения, — 1.0 МГц в системном масштабе, 5–7 МГц на уровне отдельного блока. Верификация: цифры относятся к трём разным опорным плоскостям. ~150 нс у QuiX — это от детектора до воздействия [C][3]; 196 нс — полная петля в непрерывных переменных [P][4]; 1 МГц у Aurora — время такта, а не задержка [D][2].
...
|---|---|---|---|---|
| Xanadu | разработчик | Канада | Единственное упреждающее управление за один такт в системном масштабе | [D][2] |
| PsiQuantum | разработчик | США | 300-мм коммутаторы на BTO, V&V от DARPA | [D][1] |
| QuiX Quantum | разработчик | Нидерланды | Стоечный блок упреждающего управления, ~150 нс | [C][3] |
| Lumiphase | поставщик | Швейцария | Энергонезависимая матрица на BTO | [D][5] |
...
2026-07-22 · PsiQuantum · QBI Stage C, V&V коммутаторов на BTO · USD 125 M · DARPA [P][8]
...
**ИС и стандарты.** По состоянию на 2026-09-04 в датированных базах не обнаружено патентного семейства по упреждающему управлению; датированным достижением является вентильная матрица на BTO [D][5].

**Дорожные карты и послужной список.** Xanadu — 1 МГц (выполнено 2025-01, с тех пор без улучшений) [D][2]; QuiX в 2025-07 назвала быстрое упреждающее управление блокирующим фактором 2026 года и установила блок 2026-06-02 [C][3]; PsiQuantum не опубликовала ни одного аппаратного результата за 2026 год [P][8]. Только QuiX поставила ровно то, что сама и обозначила.
