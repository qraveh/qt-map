...
## Роль в стеке
Узел принадлежит пути платформы на сверхпроводниковых трансмонах (Google, IBM, Rigetti, IQM, OQC, USTC, Fujitsu) и требует трансмонного носителя. Он заменяет кросс-резонанс на фиксированных частотах; цена перехода — перепроектирование каждой ячейки «кубит — каплер — кубит», примерно удвоенное число линий смещения и новый стек калибровки; именно на этот размен пошла IBM в Nighthawk после десятилетия на кросс-резонансе [C][6]. Вклад в такт пренебрежимо мал: производный такт = max(гейт, считывание, транспорт) равен 1.1 µs на цикл коррекции ошибок против гейта в 0.048 µs [D][4], так что этот узел двигает бюджет ошибок, а не такт. Соседнего пустого слота в графе технологий нет.
