...
Изготавливаемый в том же 300 mm CMOS-стеке, что и сам кубит, сенсор не добавляет материаловедческого риска; затворный ящик, которому нужны лишь электрод и контур, — предпочтительная для фабрики (foundry) форма. Приборную часть покупают, а не строят: SHFQC-LRT от Zurich Instruments даёт взвешенное интегрирование ≤32 µs [C][3], а в списке заказчиков OPX1000 от Quantum Machines значатся девять спиновых команд, включая Diraq, HRL, imec и Equal1 [C][4]. Частотное мультиплексирование удерживает число линий на сублинейном уровне: стена на 10³ — это разнос контуров по полосе; на 10⁴–10⁶ это суммарная полоса считывания и число усилителей, а не сам сенсор. Контроллер HRL на 4 K (366 ЦАП, ≤3.5 W) показывает, что тракт можно перенести внутрь криостата [D][5].
...
| Quantum Machines | поставщик | Израиль | Тракт считывания для девяти спиновых команд | [C][4] |
...
**Рынок и цепочка поставок.** Zurich Instruments и Quantum Machines поставляют тот радиочастотный тракт, который почти каждая спиновая команда покупает, а не делает сама [C][3], [4]; вторая узкая точка — криогенное усиление. Работает на G2, G3, G7.
...
**Дорожные карты и послужной список.** Diraq (2026-08-27 · 150 k физических, 1 k логических к 2029 · не выполнено; в релизе от 2026-07-09 говорилось «тысячи к 2029») [R][G:DIRAQ-FUNDING]. Zurich Instruments (2026-03-09 · ZQCS на «несколько тысяч кубитов» · поставляется) [P][6]. Ни один поставщик не обещает датированного темпа считывания; приборы отгружаются в срок, а число кубитов — нет.
