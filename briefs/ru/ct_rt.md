...
## Физика и пределы
Связывающее ограничение — бюджет охлаждения, а не когерентность. Каждая линия несёт пассивную теплопроводную нагрузку плюс активную — от ~60 dB ослабления, опускающего джонсоновский шум с 300 K до фотонного дна 10 mK; бо́льшая часть этого тепла сбрасывается на 4 K и на ступень испарителя (still). Холодопроизводительность каждой ступени фиксирована — Bluefors XLD400 даёт примерно 1 W при 4 K и микроватты на камере смешения, — поэтому линии масштабируются как O(N) против константы. Измеренный бюджет у Krinner et al. позволяет держать в этом криостате 50 кубитов при 14 mK, максимум около 150 [D][406]; всё, что выиграно с тех пор, — более тонкий кабель и более плотная упаковка, а не новая физика. Отказ носит когерентный характер: перекрёстные наводки и фазовый дрейф проявляются как ошибка управления, поэтому такие машины деградируют через потребность в перекалибровке, а не через декогеренцию. Что сдвигает предел: мультиплексирование нескольких кубитов на один проводник либо перенос синтеза сигналов внутрь криостата.

## Инженерное состояние (state of the art)
| Год | Показатель | Кто | Тег+ключ |
|---|---|---|---|
| 2019 | 50 кубитов на один XLD400 по измерениям; тепловая верхняя граница ~150 | ETH Zürich | [D][406] |
| 2023-12 | 1,121 кубит на одном криостате (Condor) — крупнейшая когда-либо разведённая таким способом система | IBM | [C][216] |
| 2025-11-10 | 8 каналов на один шлейф, +50% на порт, 1,536 линий в XLDsl | Delft Circuits | [C][407] |
| 2026-06-16 | KIDE специфицирован на >4,000 ВЧ-линий и >1,000 кубитов | Bluefors | [C][228] |

Разводка задаёт то N, при котором ошибки имеет смысл корректировать, но не сам предел: доминирующим членом остаётся двухкубитная неточность — Willow, 105 кубитов при среднем 2Q 99.88% и цикле 1.1 µs [D][1].

## Производство, материалы и цепочка поставок
Выход годных здесь механический (разъёмы на порт, расслоение шлейфа, тепловая привязка) и тепловой, а не литографический; стойки почти целиком COTS. Тонкий слой — криогенная разводка: Delft Circuits является ведущим независимым поставщиком шлейфов и поставляет их внутрь систем Bluefors [C][407]. Риск сконцентрирован в рефрижераторах: в 2023 г. Bluefors поглотила импульсные трубы и гелиевые установки Cryomech, забрав себе ступень под блоком растворения, при совокупной выручке выше EUR 160 M [C][408]; Oxford Instruments — единственная сопоставимая коммерческая альтернатива, ULVAC — японский третий источник с 2025 г. [C][409]. Экспортный контроль режет асимметрично: рефрижераторы выше порога BIS (≥600 µW при 0.1 K, 48 h) подпадают под ECCN 3A904, а 3A901.a контролирует КМОП, спроектированный для работы при ≤4.5 K, чем захватывает альтернативу в виде cryo-CMOS, тогда как комнатнотемпературные стойки не контролируются [G][225].

## Управление, считывание и нагрузка на ввод-вывод
От двух до четырёх линий на кубит: возбуждение, поток и линии считывания, каждая из которых мультиплексирует 5–10 кубитов. На 10³ архитектура работает и занимает бо́льшую часть крупного криостата; >4,000 линий у KIDE специфицированы под >1,000 кубитов, то есть примерно по четыре на кубит [C][228]. На 10⁴ дорожная карта по шлейфам (40,000+ линий к 2029 г.) — это экстраполяция без промежуточной демонстрации [R][407], и пределом становится глубина мультиплексирования, а не число кабелей. На 10⁶ комнатнотемпературного пути не существует; любая дорожная карта дальше ~10⁴ предполагает управление внутри криостата или разбиение по нескольким криостатам. Комнатнотемпературный круговой проход добавляет ещё и задержку кабеля плюс обработку в стойке масштаба µs [C][410] — против цикла Willow в 1.1 µs [D][1].

## Роль в стеке
Обслуживает пути платформ на трансмонах, на бозонных кот-кодах/GKP и на двухрельсовом стирании — всё сверхпроводниковое дерево — и не требует ничего выше по стеку, в чём и состоит его коммерческая сила. Каждый из его заменителей (cryo-CMOS при 4 K, SFQ при милликельвинах, ЦАП магнитного потока на кристалле) обходится в новую криогенную квалификацию, и ни один не вытеснил его на масштабе: милликельвиновый SFQ-модуль SEEQC достигает точности 1Q выше 99% и до 99.9% на пяти кубитах [D][229]; контроллер HRL при 4 K управляет 18 кремниевыми спиновыми кубитами при Λ₅/₃ = 4.7 [D][163]; IBM сообщает о паритете ASIC потокового смещения на cryo-CMOS с комнатнотемпературной электроникой на 156-кубитном Heron R2, но только в виде конференционных тезисов [C][411]. Комнатнотемпературная разводка не добавляет времени ни гейту, ни считыванию, поэтому производный раунд остаётся 0.65 µs — внутри измеренного цикла QEC 1.1 µs [D][1]. Соседний пустой слот — мультиплексированный криогенный слой управления, доказанный на масштабе массива.
...
|---|---|---|---|---|
| Bluefors | поставщик | Финляндия | Рефрижераторы с интегрированными жгутами; KIDE на >4,000 линий | [C][228] |
| Delft Circuits | поставщик | Нидерланды | Шлейф Cri/oFlex, 8 каналов на шлейф, внутри Bluefors XLDsl | [C][407] |
| Quantum Machines | поставщик | Израиль | Стойки управления OPX и ПО контроллера | [C][412] |
| Zurich Instruments | поставщик | Швейцария | Стойки ZQCS в 256-кубитной системе Fujitsu/RIKEN | [C][410] |
| Qblox | поставщик | Нидерланды | Модульные стеки управления, более низкая цена | [C][413] |
| IBM | разработчик | США | Condor, 1,121 кубит на одном криостате; претендент с cryo-CMOS | [C][216] |
| SEEQC | разработчик | США | Милликельвиновое SFQ-управление — заменитель внутри криостата | [D][229] |

**Деньги.**
2023-03-28 · Bluefors · сделка M&A, Cryomech · сумма не раскрыта, совокупная выручка > EUR 160 M · закрыта [C][408]
2024-06-20 · Qblox · раунд Series A · USD 26 M · Quantonation, Invest-NL Deep Tech · закрыт [C][413]
2025-02-25 · Quantum Machines · раунд Series C · USD 170 M · ведущий инвестор PSG Equity · суммарно USD 280 M · закрыт [C][412]
2025-12-03 · Delft Circuits · расширение раунда Series A · EUR 8 M · DeepTech XL, HTGF, QuVest · суммарно EUR 15 M · закрыто [P][414]
2026-06-17 · Quantum Machines · сделка M&A, PCB Engineering (Венгрия) · сумма не раскрыта · закрыта [C][412]

**Рынок и цепочка поставок.** Рынок стоек конкурентен и коммодитизируется: пять поставщиков квалифицированы под NVQLink [C][415], лучше всех капитализирована Quantum Machines с суммарными USD 280 M [C][412], Qblox на порядок меньше [C][413]. Криостаты и их разводка, наоборот, сконцентрированы, и Delft Circuits с суммарными EUR 15 M [P][414] — единственный значимый независимый поставщик шлейфов, то есть единая точка отказа. Узел оплачивают G2, G3, G7; не G4.

**ИС и стандарты.** Датированного подсчёта патентных семейств по криогенным ВЧ-кабелям или комнатнотемпературному управлению ни в одной поименованной базе не найдено — датированного факта нет. Формального стандарта нет; ближайшее к нему — NVQLink, который с 2025-10-28 задаёт общий интерфейс со стороны GPU для пяти поставщиков управления [C][415], тогда как импульсный стек у каждого остаётся проприетарным.

**Дорожные карты и послужной список.** Delft Circuits (2025-11-10, на 2029): 40,000+ линий на криостат, промежуточных вех нет [R][407]. Bluefors (2026-06-16, срок не указан): KIDE на >4,000 линий, только страница продукта [C][228]. Zurich Instruments (2026-03-09, срок не указан): «несколько тысяч кубитов» [C][410]. Оба поставщика кабельных решений поставили каждый заявленный ими шаг по плотности; тем не менее 2029 г. — это линейная экстраполяция.

**Стратегическое прочтение.** Если комнатнотемпературная разводка дотянет до 10⁴ кубитов, бюджет управления остаётся у поставщиков стоек и у Bluefors, а cryo-CMOS и SFQ так и останутся исследовательскими программами; проигрывают SEEQC и команды криогенных ASIC. Если плотность застрянет у 4,000–5,000 линий, переговорная сила уходит внутрь криостата, где ни один комнатнотемпературный поставщик не владеет ИС, — а сильнейшие результаты по cryo-CMOS принадлежат IBM и HRL, теперь одной компании. Экспортный контроль играет на стороне действующего решения [G][225].
