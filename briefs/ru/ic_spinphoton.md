...
Каждый узел — это спиновый кубит в твёрдом теле: азот-вакансионный или кремний-вакансионный центр в алмазе либо T-центр в кремнии, чей оптический переход излучает фотон, запутанный со спином. Фотоны от двух узлов интерферируют на светоделителе; картина срабатывания детекторов геральдирует спин-спиновую запутанность, не требуя доверия к каналу, поэтому потери стоят попыток, а не точности (fidelity). Delft продемонстрировал первый геральдированный твердотельный вариант на дистанции 3 m в 2013 г. [D][574]; всё, что было после, вталкивало этот протокол в проложенное телекоммуникационное волокно.
...
## Физика и пределы
Предел задаётся произведением эффективностей сбора, преобразования и передачи, которое входит в темп квадратично для двухфотонных протоколов и линейно для однофотонного протокола, применяемого Delft на городском масштабе [D][575]. Лишь малая доля фотонов излучателя попадает в когерентную бесфононную линию, а проложенное городское кольцо длиной 35 km стоит ещё 17 dB [D][177]. Длина волны решает, нужно ли преобразование: NV на 637 nm и SiV на 737.2 nm его требуют (Harvard преобразует в 1,350 nm), тогда как T-центр излучает изначально вблизи 1,326 nm, минуя эту ступень, но расплачиваясь более слабым переходом [P][295]. Спиновая когерентность ограничением не является: ядерные памяти удерживают запутанность дольше 2 s [D][177] — намного дольше кругового обхода по волокну за 0.25–0.35 ms. Единственный рычаг, сдвигающий предел, — усиление резонатором: волоконный микрорезонатор вокруг одного NV поднял резонансный сбор примерно вдесятеро, с ~0.05% до ~0.5%, сохранив эхо-когерентность выше 100 µs [D][G:QUTECH-NV-CAVITY-2026-01]; ещё два таких шага выводят на режим ионных ловушек.

## Инженерное состояние (state of the art)
| Год | Показатель | Кто | Тег+ключ |
|---|---|---|---|
| 2024-04 | NV, узлы в 10 km друг от друга по 25 km проложенного волокна с преобразованием в телеком-диапазон L: F = 0.534 при 0.022 s⁻¹ | QuTech/Delft | [D][575] |
| 2024-05 | SiV: F = 0.69(7) между ядерными памятями по проложенному кольцу 35 km в Boston; 0.86(3) электрон-электрон на 20 m, ≤1 Hz | Harvard | [D][177] |
| 2024-06 | T-центр, два модуля в отдельных криостатах по волокну ~40 m: белловская F = 0.60(8), ~7.5 mHz; последовательность телепортированного CNOT с постселекцией, точность гейта не приведена | Photonic Inc. | [P][295] |
...
| Photonic Inc. | Разработчик | Канада | Модули на T-центрах в кремнии, изначальное излучение в телеком-диапазоне | [P][295] |
| Harvard University | Исследования | США | Нанофотонные узлы на SiV по проложенному кольцу 35 km | [D][177] |
...
**Дорожные карты и послужной список.** Photonic Inc. (обещано в 2023 г. · распределённая отказоустойчивость «в течение пяти лет», ~2028 · статус на 4 сентября 2026 г.: многоузловой системы нет, линия 2024 г. по-прежнему с постселекцией), при цели ~200 kHz и 99.8% [R][576] — это семь порядков по темпу и 40 пунктов выше её же опубликованного результата, то есть архитектурная цель, а не дорожная карта. QuTech дорожной карты не публикует и выдаёт результат уровня Nature примерно раз в год — достоверен по факту поставки и честен в отношении темпов.
